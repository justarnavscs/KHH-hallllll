...
import { Building2, User, Mail, Phone, Building, ClipboardEdit, Send, CheckCircle, AlertTriangle, Loader2 } from 'lucide-react';
import { db, isFirebaseConfigured, mockDb } from '../firebaseClient';
import { collection, addDoc } from 'firebase/firestore';

export default function BulkForm() {
  // Input fields state (Repurposed for Corporate Real Estate)
  ...
  const validate = () => {
    const errors = {};
    if (!name.trim()) errors.name = 'Representative name is required.';
    if (!company.trim()) errors.company = 'Company or Organization name is required.';
    if (!email.trim()) {
    ...
    if (!quantity.trim()) {
      errors.quantity = 'Estimated unit count is required.';
    } else if (isNaN(Number(quantity)) || Number(quantity) <= 0) {
      errors.quantity = 'Quantity must be a positive number.';
    } else if (Number(quantity) < 2) {
      errors.quantity = 'Minimum 2 units required for institutional/corporate pricing.';
    }

    if (!requirements.trim()) {
      errors.requirements = 'Please describe your project requirements.';
    } else if (requirements.trim().length < 15) {
      errors.requirements = 'Please provide more details about your inquiry.';
    }
  ...
  const handleBulkSubmit = async (e) => {
    ...
    const payload = {
      ...
      phone: phone.trim(),
      requested_units: Number(quantity),
      project_requirements: requirements.trim(),
      lead_type: 'Corporate/Institutional',
      lead_status: 'New'
    };

    try {
      if (isFirebaseConfigured) {
        // Targeted Panchratna Project Inquiry Collection
      ...
      fetch('/api/updateSheets', {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            email: payload.email,
            phone: payload.phone,
            units: payload.requested_units,
            requirements: payload.project_requirements
          }
        })
      }).catch((sheetErr) => console.error('Panchratna Sheets sync failed:', sheetErr));
      ...
      setError('An error occurred during submission. Please try again or call our head office.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 md:p-10 shadow-md shadow-[#EFEAE2] relative overflow-hidden border border-[#EAE5DC] max-w-5xl mx-auto">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left column - Construction context */}
        <div className="lg:col-span-4 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <h3 className="text-2xl md:text-3xl font-extrabold text-[#1E293B] tracking-tight uppercase">
              Project Partnerships
            </h3>
            <p className="text-sm text-[#64748B] leading-relaxed">
              Inquire about bulk residential bookings, corporate office space acquisitions, or institutional investment opportunities. We offer tailored structural solutions and tiered pricing for organizations looking to secure multiple assets across the Panchratna portfolio.
            </p>
          </div>
          ...
          <div className="mt-8 space-y-4 border-t border-[#EAE5DC] pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">Asset Management</h4>
                <p className="text-2xs text-[#64748B]">Professional guidance for institutional portfolios.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">MOQ: 2+ Units</h4>
                <p className="text-2xs text-[#64748B]">Exclusive benefits for corporate and bulk bookings.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">RERA Verified</h4>
                <p className="text-2xs text-[#64748B]">Full legal compliance for all institutional acquisitions.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleBulkSubmit} className="lg:col-span-8 space-y-5">
          {success && (
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 flex items-start gap-3 animate-fade-in">
              ...
              <div>
                <h4 className="text-sm font-bold">Inquiry Successfully Logged</h4>
                <p className="text-xs text-[#64748B] mt-0.5">Your institutional inquiry has been registered. Our corporate relations team from Panchratna Constructions LLP will reach out to you shortly.</p>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. Infratech Solutions"
                  className={inputClasses}
                />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. relations@company.com"
                  className={inputClasses}
                />
              </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            <div>
              <label htmlFor="bulk-quantity" className="block text-2xs font-bold text-[#64748B] uppercase tracking-widest mb-1.5">
                Required Inventory Volume (Units / Floors)
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[#94A3B8]">
                ...
                <input
                  ...
                  placeholder="Minimum inquiry: 2+ units"
                  min="2"
                  className={inputClasses}
                />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <textarea
                  ...
                  placeholder="Describe your corporate requirements (e.g. combined floors at Altius, dedicated office wings, investment timelines, or specialized structural modifications for institutional use)..."
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={submitting}
              className="w-full btn-neon-emerald py-3 px-6 rounded-xl flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {submitting ? (
                <>
